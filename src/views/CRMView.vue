<template>
  <div class="page-layout">
    ...
    <!-- PAGE HEADER — padrão idêntico ao Dashboard -->
    ...
    <div v-if="tab === 'kanban'" class="kanban-board">
      <div v-for="e in ETAPAS" :key="e.id" class="kb-col">
        ...
        <div class="kb-cards">
          <div v-for="l in byEtapa(e.id)" :key="l.id" class="kb-card" @click="openLead(l.id)">
    ...
    <div v-if="tab === 'tabela'" class="tabela-wrap">
      <div class="tabela-filters">
        <div class="search-box">
          ...
          <input v-model="search" class="search-input" placeholder="Buscar lead..." />
        </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-if="!listaFiltrada.length">
              <td colspan="9" style="text-align:center;color:var(--text-tertiary);padding:2rem;font-size:.875rem">Nenhum lead encontrado</td>
            </tr>
            <tr v-for="l in listaFiltrada" :key="l.id" style="cursor:pointer"
              :class="{ 'row-sel': selected.has(l.id) }"
              @click="openLead(l.id)">
              ...
              <td><span class="pri-tag" :class="`pri-${l.prioridade}`">{{ l.prioridade }}</span></td>
...
// Computed cacheados para performance
...
<style scoped>
/* ── Layout padrão — idêntico ao Dashboard ── */
.page-layout {
...
/* Page header — mesmo padrão do Dashboard */
.page-header {
  display: flex;
...
.sel-bar {
  ...
  border-radius:var(--radius-lg); padding:.625rem 1rem;
}
...
.tabs {
  ...
  border-radius:var(--radius-lg); padding:.25rem; width:fit-content;
}
.tab {
...
.kb-col {
  ...
  border-radius:var(--radius-lg); display:flex; flex-direction:column; overflow:hidden; min-width:0;
}
.kb-head {
  display:flex; align-items:center; justify-content:space-between;
  padding:.625rem .75rem; border-bottom:1px solid var(--border-subtle);
}
...
.kb-card  {
  background:var(--bg-elevated); border:1px solid var(--border-default);
  border-radius:var(--radius-md); padding:.75rem; cursor:pointer;
...
.kb-add     {
  display:flex; align-items:center; justify-content:center; gap:.25rem; width:100%;
  padding:.4rem; background:transparent; border:1px dashed var(--border-default);
  border-radius:var(--radius-md); color:var(--text-tertiary); font-size:.75rem;
...
.search-box {
  ...
  background:var(--bg-elevated); border:1px solid var(--border-default);
  border-radius:var(--radius-md); padding:.4rem .75rem; flex:1; min-width:200px;
}
.search-box svg { flex-shrink:0; color:var(--text-tertiary); }
.search-input { background:transparent; border:none; outline:none; font-family:var(--font-body); font-size:.875rem; color:var(--text-primary); flex:1; }
...
.pri-tag   { font-size:.75rem; font-weight:700; text-transform:uppercase; }
...
.drawer {
  ...
  box-shadow:var(--shadow-lg); z-index:801;
...
.drawer-footer {
  ...
  padding:.875rem 1.25rem; border-top:1px solid var(--border-default); flex-shrink:0;
}

/* Script box */
.script-box {
  background:var(--bg-overlay); border:1px solid var(--border-default);
  border-radius:var(--radius-md); padding:.75rem; font-size:.8rem;
